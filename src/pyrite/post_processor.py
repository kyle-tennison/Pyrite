...
class PostProcessor:
    ...
    def output_solved(self, nodal_forces: np.ndarray, nodal_displacements: np.ndarray, nodes: list[Node]):
        """Outputs the solved system to output.csv"""

        with open("output.csv", 'w') as f:
            f.write("id, x, y, ux, uy, fx, fy\n")
            ...
            for node in nodes:
                ...
                f.write(f"{node.id},{node.x},{node.y},{ux},{uy},{fx},{fy}\n")
    ...
    def show(self, input_file: str):
        ...
        initial_plot.triplot(begin_points[:,0], begin_points[:,1], tri_begin.simplices)
        ...
        solved_plot.triplot(solved_points[:,0], solved_points[:,1], tri_begin.simplices)
